...
namespace PepperoniBattleRoyale.Content.Projectiles
{
    ...
    {
        private const string ChainTexturePath = "PepperoniBattleRoyale/Content/Projectiles/SandballChain";
        ...
        public override void AI()
        {
            ...
            int launchTimeLimit = 15; //How much time the flail can go before retracting
            float launchSpeed = 14f; //How fast the flail goes
            float maxLaunchLength = 800f; //How far the flail's chain can stretch
            float retractAcceleration = 3f; //How quickly the flail accelerates back towards the player
            float maxRetractSpeed = 10f; //The max speed the flail retracts at
            float forcedRetractAcceleration = 6f; //How quickly the flail retracts while being forced to
            float maxForcedRetractSpeed = 15f; //My god, why are there so many variables
            float unusedRetractAcceleration = 1f; //why tf is this here if it's unused
            float unusedMaxRetractSpeed = 14f;
            int unusedChainLength = 60;
            int defaultHitCooldown = 10; //How often the flail hits when resting or retracting
            int spinHitCooldown = 20; //How often the flail hits when spinning
            int movingHitCooldown = 10; //How often the flail hits when moving
            int ricochetTimeLimit = launchTimeLimit + 5;

            //Scales speeds and accelerations by the player's melee speed, makes the weapon more responsive
				...
				case AIState.LaunchingForward:
					{
						...
						if (player.controlUseItem) // If the player clicks, transition to the Dropping state
						{
							...
							if (Main.myPlayer == Projectile.owner)
								Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 3, ModContent.ProjectileType<SandballShot>(), Projectile.damage, Projectile.knockBack, Main.myPlayer);
						...
						if (shouldSwitchToRetracting)
						{
							...
							Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 2, ModContent.ProjectileType<SandballShot>(), Projectile.damage * 3, Projectile.knockBack, Main.myPlayer);
